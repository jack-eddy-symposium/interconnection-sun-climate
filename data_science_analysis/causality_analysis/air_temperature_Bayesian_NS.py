...
def single_lat_lon_model(
    ...
    if oni is not None:
        ...
        target_variable_estimate = target_variable_estimate + oni_gain * jnp.interp(
            orbits - jnp.abs(oni_shift), orbits_oni, oni.normalized.values
        )
    ...
    if cr is not None:
        ...
        target_variable_estimate = target_variable_estimate + cr_gain * jnp.interp(
            orbits - jnp.abs(cr_shift), orbits_cr, cr.normalized.values
        )
...
if __name__ == "__main__":
    ...
    ## Read climate data
    geo_data = xr.open_dataset(DATA_INPUT_FILE).sel(level=2)
    ...
    for coarsen_level in pbar1:
        ...
        geo_data_coarse = geo_data.coarsen(
            lon=coarsen_level, lat=coarsen_level, time=DATA_SMOOTHING, boundary="pad"
        ).mean()
        ...
        for lon_index in pbar2:
            lon = geo_data_coarse.lon.values[lon_index]
            pbar2.set_postfix_str(
                f"Processing {lon}° longitude",
                refresh=True,
            )
            ...
            for lat_index in pbar3:
                lat = geo_data_coarse.lat.values[lat_index]
                pbar3.set_postfix_str(
                    f"Processing {lat}° latitude",
                    refresh=True,
                )

                target_variable  = geo_data_coarse.air.sel(lat=lat,lon=lon).values

                # Run with only co2 increase and seasonal variation
                output_file = os.path.join(
                    output_folder,
                    f"{DATA_LABEL}_smoothing{DATA_SMOOTHING}_level{coarsen_level}_co2_season_lat_{lat_index}_lon_{lon_index}.pkl",
                )
